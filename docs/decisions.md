...
- Explicit non-goals for v1: multi-currency, receipt-photo attachments, non-member guest participants, import from external expense-splitting apps. All noted for the backlog; the schema keeps `currency` per-trip so multi-currency is an additive change later.
