...
**Roadmap next:** **Trip Workspace v2 is COMPLETE** (V2.0–V2.7 all shipped & pushed); a **v2.8 workspace-chrome iteration** is pushed to origin as **[PR #3](https://github.com/Aedrand/caravan/pull/3)** (branch `feat/workspace-v2.8-chrome`, not merged — awaiting review; see next section). → then **M6 — v1.0 hardening & release** over the v2-inclusive app.

## This session (2026-06-30/07-01 — Trip Workspace v2.8 chrome iteration, on a branch)

Pulled the latest **v2.8** design from the claude.ai **Caravan Design System** project (via DesignSync) and applied the workspace-chrome iteration. Committed to branch **`feat/workspace-v2.8-chrome`** (`ba59d55` code + `020c62c` docs) and **pushed to origin as [PR #3](https://github.com/Aedrand/caravan/pull/3)** — **not merged** (awaiting owner review). 9 files, +108/−118 (a net declutter). Gates green: web + server typecheck, biome lint, `pnpm -r build`, **e2e 5/5**; visually verified on the Japan trip @ :5173. **origin/main @ `2fc46e9`** (the branch's base) — merge PR #3 (ff, like the earlier v2 phases) to land it.
...
- **V2.7 — workspace shell — ✅ SHIPPED (`8b111af`):** continuous-scroll shell — 240px **IndexRail** + scrolling canvas + sticky ambient map; providers lifted to the frame; swappable `useScrollSpy`. Sections **Overview → Bookings → Itinerary → Ideas → Money → Group**; **Bookings promoted to its own section above Itinerary** (owner change), Transport/Lodging groups via an extensible registry; **Overview hero** (compact `BudgetBar` + attention chips + editable `trips.bulletin` group bulletin (**migration 0008**) + recent-feed peek); itinerary day-rail + ideas-pointer removed (IndexRail replaces them), in-day shims kept; mobile keeps the bottom-tab nav. Built architect → serial foundation agent → 2 parallel surface agents → gate sweep; 344 unit, e2e 5/5. **Open reviews still pending:** MAP-PIN-DAY-COLOR + BUDGET-PLANNED-SEMANTICS — see `enhancements.md`.

**Scope guardrails (from the decisions):** **desktop-first** — *mobile UX is its own later design review, not now*. **Deferred:** the file/image upload subsystem (so hero image, image-type ideas, and file attachments wait). **Out of scope (now):** full UI localization; **public-transit routing — decided 2026-06-29 to defer to a post-v1.0 revisit** (in-app = walk/drive only; Google Maps link-out is the interim transit path; three revisit options captured in `plan.md` → "Public transit — deferred revisit"). Reuse the existing activity row + sync/feed/permissions (PD-13) — don't build parallel machinery per type.

**Next move:** **Trip Workspace v2 is COMPLETE (V2.0–V2.7, all on `main` @ `8b111af`).** Next is **M6 — v1.0 hardening & release** (see "Roadmap after v2: M6" below). The remaining v2-era owner reviews in `enhancements.md` (MAP-PIN-DAY-COLOR, BUDGET-PLANNED-SEMANTICS, ROUTING-PERF-LAZY-LOAD, TRIP-SETTINGS-DIALOG, FLIGHT-COST-FIELD, future booking types, self-hosting/privacy) are batch-review items, not blockers.
...
- **Session end 2026-07-01:** the dev server was **shut down** (owner asked to close everything out). Nothing is running on :5173/:3000 — start fresh with `pnpm dev`. The v2.8 work is safely on origin as PR #3.
