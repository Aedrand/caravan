...
| V2.5 | Routing | proxy + multi-modal engine, route lines, travel-times, modes (TD-13) — consumes V2.4 anchors as day endpoints. **Map day-legibility (owner ask 2026-06-28):** color map pins by the **day** they belong to (not activity category) so a glance tells you which day a pin is — pairs with day-colored route lines (a day's pins + its route share one color). *Reconsiders the V2.3 category-tint* — open: replace category coloring outright, or keep category as a secondary cue (glyph/icon in the pin) with day as the fill. See `enhancements.md`. |
