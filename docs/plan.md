...
| V2.3 ✅ | Plan View v2 build — **SHIPPED `e721ca8`** | order-driven **progression rail**: two-line rows, numbered square stamps (hollow when unplotted) synced to **numbered map pins**, inline note/checklist rows (toggles), est-cost chips, inline-editable day subtitle, "N stops · ~$est" summary, drag-to-resequence; idea lists + freeform note/checklist idea types on Decide; typed-item form dialog. Built in today's tabbed shell (foundation + 3-surface-agent pass). **Polish SHIPPED 2026-06-28** (`a4c2ac6`): category-tint map pins (JS token bridge — `map/pin-tint.ts`), inline cost/time edit on rail rows, drag-ideas-between-lists; then map clustering tuned to minimal (`3c1744b`→`6b6b641`: `clusterRadius` 12 / `clusterMaxZoom` 9, so day pins stay separate until a full trip-overview zoom). **Still deferred:** travel-time labels + 🏨 home-base anchors (V2.4), route-mode toggle (V2.5). 259 unit, e2e 5/5 green |
| V2.4 ✅ | Bookings + day anchors — **SHIPPED `e97882a`** | flight/lodging on the activities table (9 nullable cols) + 6 home-base cols on `days`, migration **0006**; shared `bookings.ts` (`computeBookingDerivedEntries` + `deriveAnchors` override-then-computed); booking form (flight = dual autocomplete, lodging = checkout-day + times + total cost); rail `BookingBody` + read-only derived check-out/arrival rows + day-header anchor chip with **manual home-base override** (✎/×); map dual flight pins + single lodging pin (un-numbered, tinted). Owner forks: 1-leg flights · manual anchor override YES · total-stay cost. 281 unit, e2e 5/5 |
| V2.5 ✅ | Routing — **SHIPPED `b236af5`** | keyless `/api/route` proxy (FOSSGIS Valhalla default, key-optional ORS) + `route_cache` + rate-limit + graceful-off; shared route contract + `effectiveRouteMode`; migration **0007**; env `ROUTING_PROVIDER/URL/ORS_KEY/...`. **Day route lines** (day-colored) on the map + **travel-time labels** between stops on the rail + walk/drive **mode toggle** (per-day override + trip default). Consumes V2.4 anchors as day endpoints. Provider/mode/perf decisions made autonomously (owner away), configurable + logged. 329 unit, e2e 5/5. **Open (now active — routes ARE day-colored):** also color map pins by day? → `enhancements.md` MAP-PIN-DAY-COLOR. **⚑ Default host DOWN (2026-06-29):** `valhalla1.openstreetmap.de` is unreachable (the host itself, not a code bug — the proxy + graceful-off work, so walk/drive legs just render blank); near-term fix = wire a keyless **OSRM** adapter (FOSSGIS `routing.openstreetmap.de`, `routed-foot`/`routed-car`, confirmed up) as the default, `ROUTING_URL` escape hatch unchanged → see `enhancements.md` 2026-06-29. |
| V2.6 ✅ | Money — **SHIPPED `748e732`** | **client-only** (bridge cols `estimatedCostMinor` + `expenses.activityId` already existed): "Log as expense" on the rail row → prefilled `ExpenseFormDialog` (user confirms payer+split, never auto-creates debt), idempotent ("Already logged" once linked); `budget.ts` selectors + `BudgetBar` (planned-vs-actual comparison, compact-ready for the V2.7 hero); estimate KEPT after convert. 340 unit, e2e 5/5. **One owner-confirm teed up:** planned/actual double-count rule (Planned = all dated estimates as a *comparison* vs Actual; a one-line selector flip) → `enhancements.md` BUDGET-PLANNED-SEMANTICS. |
| V2.7 ✅ | Workspace shell — **SHIPPED `8b111af`** | tabbed workspace → single **continuous-scroll shell**: 240px **IndexRail** + scrolling canvas + sticky ambient map; FocusedDay/MapSelection/Routing providers **lifted** to the frame; swappable **`useScrollSpy`** (IntersectionObserver, root=canvas) syncs rail↔scroll + drives focused day. Sections **Overview → Bookings → Itinerary → Ideas → Money → Group**. **Bookings is its OWN top-level section ABOVE Itinerary** (owner change from the approved mockup): full-width, Transport(flights)/Lodging(hotels) via extensible `BOOKING_CATEGORIES`; old itinerary strip removed, in-day shims kept. **Overview hero**: identity+countdown, compact `BudgetBar`, attention chips, editable **group bulletin** (`trips.bulletin`, **migration 0008**), recent-feed peek. Itinerary day-rail + ideas-pointer removed; mobile keeps bottom-tab. Built architect → serial foundation → 2 parallel surface agents → gate. 344 unit, e2e 5/5 |
| V2.8 ⏳ | Chrome iteration — **PR #3** (`feat/workspace-v2.8-chrome`, pushed, not merged) | Visual-only pass applying the claude.ai Caravan Design System **v2.8** rules to the workspace: **baseline-rule section headers** (emoji→**Lucide** `icon` prop + 40px cream tile + 2px ink baseline underline; all six sections), **three-line non-sticky day headers** (rotated `DAY n` stamp / dotted-underline subtitle / stats), **declutter** (dropped dup Today/Trip-start from the Itinerary header, removed the redundant Overview budget block, removed inline pencils on bulletin + day subtitle), and a **no-emoji-in-chrome** sweep (attention chips + Bookings group glyphs + bulletin pushpin → Lucide). **Bookings kept standalone** (owner reaffirmed vs the v2.8 shell-ref's in-Itinerary strip). No migration. Gates green incl. e2e 5/5; visually verified @ :5173. |
...
### Public transit — deferred revisit (post-v1.0) `[S]`

> **Decided 2026-06-29 (owner).** Public-transit routing is **out of scope for now** — in-app routing stays **walking/driving only** (open stack). The interim transit story is the existing **Google Maps link-out** (keyless deep-link → opens the route in Google Maps; no API key, no licensing issue, no second map). **Revisit after the rest of the roadmap.**

**Why deferred (coverage reality):** worldwide transit routing with real schedules is effectively a Google-only dataset. The open engines (Transitous/MOTIS, Navitia, OpenTripPlanner) only cover where open GTFS exists — Western Europe, major US metros, Tokyo — and go blank for e.g. Kansai, the Shinkansen, and Seoul; **self-hosting doesn't fix this** (the data isn't published openly). And Google's terms forbid drawing its transit data on a non-Google basemap, so in-app Google transit requires a second map renderer. The link-out sidesteps all of it.

**When revisited, weigh three options (cheapest → heaviest):**
1. **Google Maps Embed API (iframe panel)** — a real in-app Google transit *view* in a panel; free/unmetered, ToS-compliant, **no second renderer**. Tradeoff: a boxed Google map, not blended into our MapLibre map.
2. **Open transit provider in-app** — Transitous/MOTIS (keyless) → self-host MOTIS to harden; draws transit lines on *our* map via the routing-adapter pattern, no licensing issue. Tradeoff: coverage gaps (no Kansai/Shinkansen/Seoul).
3. **Separate Google Maps renderer** — Google transit blended into a full Google basemap. Tradeoff: rebuilds our most complex component + a permanent ~2× map-maintenance tax + per-load billing. Only if the integrated Google look/data is a hard requirement.

Inbox pointer + full rationale (coverage comparison, licensing, lift estimates) in `enhancements.md` (2026-06-29 routing/transit entry).
...
| OpenFreeMap / Photon / **routing** public instances degrade (no SLA) | Maps UX | Server-side proxy + cache; env-swap to keyed providers; PMTiles/self-Photon documented exit (TD-5). **Materialized 2026-06-29:** the default routing host (`valhalla1.openstreetmap.de`) went fully unreachable → switching the keyless default to FOSSGIS OSRM (`enhancements.md`); reinforces that public keyless = no SLA and the `ROUTING_URL` escape hatch is the durable answer |
