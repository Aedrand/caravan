...
> _Note (2026-06-28): names of external/competitor travel projects have been redacted from this internal research record — they appear as "Competitor A–H", with repo handles removed. Neutral self-hosted examples cited for packaging/ops/licensing patterns (Mealie, Vikunja, Grist, Actual Budget, Immich, Nextcloud, etc.) are not competitors and are unchanged._

> **ERRATA (2026-06-10, post fact-check — see `fact-check.md`):** This file's characterization of Competitor A as "individual-planner-with-sharing, not group-coordination-first" is **refuted**. Competitor A's own README bills it as a real-time collaborative *group* planner — WebSocket co-editing, polls, group chat, per-person expense splits, day check-ins. The "niche is genuinely open" conclusion (TL;DR and Prior Art sections below) therefore does not hold; Caravan's positioning must rest on differentiation (ops simplicity, BYO-AI depth, decision-flow UX, design), not on an empty niche. Competitor A's other attributes (stars, license, stack, version, awesome-selfhosted status) were all verified accurate. Packaging-norms and licensing findings are unaffected.

## TL;DR

- **Single container + SQLite is the gold-standard starting point** for small-scale self-hosted apps in 2026; Mealie, Vikunja, Grist, Actual Budget, and Competitor A all prove it. Add a compose with an external DB only when the workload demands it.
- **The collaborative group travel-planner niche is genuinely open.** Competitor B (3.3 k stars, GPL-3.0, Django+SvelteKit) is the closest FOSS match but is personal-journal-first, not group-coordination-first. Competitor A (5.6 k stars, AGPL-3.0, Node+React+SQLite) overlaps heavily on features and should be studied carefully to differentiate Caravan.
- **AGPL-3.0 is the right license** for a self-hosted web app that the owner wants to keep perpetually free. It closes the "hosted fork" loophole, matches what Plausible, Competitor C, Competitor A, and Nextcloud chose, and carries FSF blessing for network-delivered software.
...
- **Documentation norm:** Provide a sample Caddyfile and a sample Traefik labels block in the docs. Competitor B ships a dedicated `docker-compose-traefik.yaml`.
  - Source: https://github.com/redacted/Competitor B (accessed 2026-06-10)
...
- Competitor B has an official demo at `demo.competitor-b.app` and screenshots in the README — this is cited as a driver of its 3.3 k star count.
  - Source: https://competitor-b.app/ (accessed 2026-06-10)
- The Competitor A project shows GIF demos and a live demo URL prominently in its README — 5.6 k stars suggests this works.
  - Source: https://github.com/redacted/Competitor A (accessed 2026-06-10)
- **Competitor A is awaiting addition to awesome-selfhosted** (open issue #2361 on the data repo as of 2026-06-10).
...
- **Differentiation for Caravan:** Competitor B is personal-journal-first with sharing bolted on. Caravan is group-coordination-first from day one (real-time sync, voting, expense splitting). The Postgres dependency is a barrier Caravan should avoid.
- **Verified:** 2026-06-10
...
- **What to learn:** Competitor A is the closest overlap to Caravan's vision. Its star count (5.6 k, higher than Competitor B) at comparable maturity validates the market.
- **What to avoid / differentiate:** Competitor A is a full-featured "enterprise-ish" planner — Caravan should be simpler and friendlier for non-technical friend groups. Competitor A's feature set may feel overwhelming; Caravan's "one tech-savvy friend deploys it" UX constraint is a meaningful differentiator. Also: Competitor A has not been listed on awesome-selfhosted yet (open issue as of 2026-06-10).
- **Warning:** Stack (Node + SQLite + TypeScript) is near-identical to what Caravan is likely to use. There is a real risk of overlap perception. Caravan must clearly articulate its group-coordination focus vs. Competitor A's individual-planner-with-sharing model.
- **Verified:** 2026-06-10

### Competitor C

- **Status:** Active. v0.19.2 released 2026-06-01. 3.7 k stars, AGPL-3.0. 1,401 commits.
  - Source: https://github.com/open-competitor-c/competitor-c (accessed 2026-06-10)
- **License:** AGPL-3.0
...
- Under **Maps and Global Positioning System (GPS)**, only two travel-adjacent apps are listed: **Competitor B** (travel tracker/trip planner) and **Competitor H** (personal flight tracking).
...
- SaaS tools in 2026 for group travel: Competitor E (free, collaborative itinerary + polls + budget), Competitor F (payment collection focus), Competitor D (consumer, no self-host), Competitor G (itinerary parsing). All are proprietary/cloud-only.
  - Source: https://www.competitor-e.com/en/alternatives/best-group-trip-planner-apps (accessed 2026-06-10)
- **The self-hosted group travel planner is an unoccupied niche.** There is no open-source, self-hostable Competitor D equivalent with group coordination features.

### Niche conclusion

The niche is open. Competitor B and Competitor A are the closest projects but both have distinct foci (personal log vs. individual planner). A group-coordination-first, self-hostable tool with real-time sync, voting, and expense splitting has no direct FOSS competition. The main risk is Competitor A's feature overlap; Caravan should explicitly position around the "friends deploys it for the group" UX, smaller scope, and simpler onboarding.
...
|---|---|---|
| Competitor B | GPL-3.0 | Copyleft; no explicit SaaS concern stated |
| Competitor A | AGPL-3.0 | Explicitly chosen to require open-source modifications when run as network service |
| Competitor C | AGPL-3.0 | Community/activist project; strong copyleft stance |
...
- https://github.com/redacted/Competitor A (accessed 2026-06-10)
...
- Competitor B uses GPL-3.0; this means a company could theoretically fork it, improve it, run it as SaaS, and keep changes proprietary.
...
2. All close FOSS peers (Competitor A, Competitor C, Plausible) chose AGPL-3.0 for the same reasons; it signals seriousness about staying FOSS.
...
- **Maps:** Leaflet (open) over Google Maps or Mapbox GL (API key/cost). Competitor A and Competitor B both use Leaflet/MapLibre.
- **First listing strategy:** Submit to awesome-selfhosted early (it requires a demo instance). A live demo at `demo.caravan.app` with a few sample trips loaded is the most effective adoption driver based on Competitor B and Competitor A trajectories.

---

## Open Questions / Unverified Claims

1. **Competitor A feature overlap is the largest competitive risk.** Need to do a deeper feature-by-feature comparison to identify genuine gaps Competitor A doesn't address (expense settlement, native mobile PWA quality, simpler UX). Unverified: whether Competitor A has expense-splitting or just budget tracking.
...
3. **Competitor B collaborative editing** — it's unclear whether its "collaborator can edit" feature is real-time or just shared-access-to-same-itinerary. Needs direct testing.
...
5. **Competitor A licensing compliance** — Competitor A uses AGPL-3.0. If Caravan borrows any code patterns or architecture from Competitor A (even paraphrased), the AGPL does not require Caravan to be AGPL (independent creation is fine), but any direct code incorporation would. Keep implementations independent.
...
3. https://github.com/redacted/Competitor B — Competitor B stars, license, stack, features, activity (GitHub, accessed 2026-06-10)
4. https://competitor-b.app/ — Competitor B official site, demo instance (official, accessed 2026-06-10)
5. https://github.com/redacted/Competitor A — Competitor A stars, license, stack, features, activity (GitHub, accessed 2026-06-10)
6. https://github.com/awesome-selfhosted/awesome-selfhosted-data/issues/2361 — Competitor A awesome-selfhosted submission pending (GitHub issue, accessed 2026-06-10)
7. https://github.com/open-competitor-c/competitor-c — Competitor C stars, license, stack, activity (GitHub, accessed 2026-06-10)
8. https://awesome-selfhosted.net/ — Travel category: only Competitor B and Competitor H listed under Maps/GPS (official, accessed 2026-06-10)
...
35. https://www.competitor-e.com/en/alternatives/best-group-trip-planner-apps — Commercial group trip planning landscape 2026 (commercial site, accessed 2026-06-10)
